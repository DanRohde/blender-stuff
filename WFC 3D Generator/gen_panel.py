...
class VIEW3D_PT_GeneratorPanel(bpy.types.Panel):
    ...
    bl_idname = "VIEW3D_PT_wfc_generator_panel"
    ...
    def draw(self, context):
        ...
        col = row.column()
        col.operator("object.wfc_toggle_button", icon='PAUSE', depress=props.paused_delayed_renderer).prop_name = 'paused_delayed_renderer'
        col.enabled = props.running_delayed_renderer
        col = row.column()
        col.operator("object.wfc_stop_button", icon='EVENT_MEDIASTOP').prop_name ="running_delayed_renderer"
        col.enabled = props.running_delayed_renderer
        ...
        if props.search_result.steps > -1 and props.search_result.search_scope == "occupancy" and props.search_result.search_operator == "max":
            ...
            row.operator("object.wfc_reset_search_result",icon="PANEL_CLOSE")
...
def render_render_result(props, layout):
    if props.render_result.cell_count > 0:
        ...
        if props.render_result.object_count != "" and not props.render_result.object_count_collapsed :
            ...
            box.label(text=f"{rr.object_total} object(s), {rr.bb_count_used} of {rr.bb_count} building block(s) is/are used:")
...
def render_seed_selection(props, prefs, row, render_allowed):
    ...
    if prefs.cherry_picking_delay > 0:
        col = row.column()
        col.operator("object.wfc_cherry_picking", icon='PLAY' if not props.cherry_picking_running else 'PAUSE', depress=props.cherry_picking_running)
        col.enabled = render_allowed and not props.search_running
    col = row.column()
    col.operator("object.wfc_auto_generate_toggle", icon='AUTO', depress=props.auto_generate)
...
def render_generate_button(props, row, render_allowed):
    if not props.progress_running and not props.running_delayed_renderer:
        ...
        if props.progress_running:
            row.operator("object.wfc_toggle_button", icon='PAUSE', depress=props.progress_paused).prop_name = "progress_paused"
            row.operator("object.wfc_stop_button", text="", icon='EVENT_MEDIASTOP').prop_name = "progress_running"
        if props.running_delayed_renderer:
            row.operator("object.wfc_toggle_button", icon='PAUSE', depress=props.paused_delayed_renderer).prop_name = "paused_delayed_renderer"
            row.operator("object.wfc_stop_button", text="", icon='EVENT_MEDIASTOP').prop_name = "running_delayed_renderer"
panels = [ VIEW3D_PT_GeneratorPanel ]
