...
class WFC3DGeneratePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if props.progress > 0: layout.row().progress(factor=props.progress, text=f"{round(props.progress*100)}% (elapsed: {round(props.progress_elapsed_time,0):.0f}s/eta: {props.progress_eta:.0f}s)", type="BAR")
        if props.render_delay > 0:
            row = layout.row()
